 ...
 * Last modified: Mon Dec 23 19:25:08 2013 mstenber
 * Edit time:     16 min
 *
 */
                              ...
                              bool exp_ok, int exp_used)
{
  ...
  /* Ok path. */
  KVASSERT(left == buf_bytes - exp_used, "wrong # left: %d (expected:%d, really used %d)", (int)left, exp_used, buf_bytes - (int)left);
...
static void test_endecode_s64(int64_t v, int buf_bytes,
                              bool exp_ok, int exp_used)
{
  unsigned char buf[128];
  unsigned char *c = buf, *d;
  ...
  KVDEBUG("test_endecode_s64 %lld", (int64_t) v);

  encode_varint_s64(v, &c, &left);
  if (!exp_ok)
    {
      KVASSERT(left == -1, "wrong result - succeeded?!?");
      return;
...
int main(int argc, char **argv)
{
  ...
  test_endecode_s64(0, 1, true, 1);
  test_endecode_s64(-1, 1, true, 1);
  test_endecode_s64(1, 1, true, 1);
  test_endecode_s64(63, 1, true, 1);
  test_endecode_s64(64, 2, true, 2);
  test_endecode_s64(-63, 1, true, 1);
  test_endecode_s64(-64, 1, true, 1);
  test_endecode_s64(-65, 2, true, 2);
  
  test_endecode_u64(23467742135246, 10, true, 7);

  test_endecode_s64(23467742135246, 10, true, 7);
